 ...
 *
 * Value operations — see Value.h for the public contracts.
 */

#include <Plain/Runtime/Frame.h>

void PLAIN_VALUE_CLEAR(struct PLAIN_VALUE* value) {
    /* User-owned object — release the frame reference. */
    if(value->type == PLAIN_TYPE_OBJECT && (value->owner & PLAIN_OWNER_USER)) {
        if(value->data != NULL) {
            PLAIN_FRAME_RELEASE((struct PLAIN_FRAME*)value->data);
        }
        value->data   = NULL;
        value->length = 0;
        value->owner  = 0;
    ...
    /* Heap-allocated data — free the buffer. */
...
PLAIN_WORD_DOUBLE PLAIN_VALUE_COPY(struct PLAIN_VALUE* destination, const struct PLAIN_VALUE* source) {
    ...
    /* Object values — share the frame, bump the reference count. */
    if(source->type == PLAIN_TYPE_OBJECT && (source->owner & PLAIN_OWNER_USER)) {
        destination->data = source->data;
        if(source->data != NULL) {
            PLAIN_FRAME_RETAIN((struct PLAIN_FRAME*)source->data);
        }
        return 0;
    }

    /* Heap-allocated data — allocate a copy. */
    if(source->length > 0 && source->data != NULL) {
        ...
        /* Zero-length data: boolean marker (NULL / 0xFF), nil, etc. */
        destination->data = source->data;
    }
...
PLAIN_WORD_DOUBLE PLAIN_VALUE_TRUTHY(const struct PLAIN_VALUE* value) {
    switch(value->type) {
        ...
        case PLAIN_TYPE_INTEGER:
            return *(PLAIN_WORD_QUADRUPLE*)value->data != 0;
        case PLAIN_TYPE_REAL:
            return *(PLAIN_REAL*)value->data != 0.0f;
