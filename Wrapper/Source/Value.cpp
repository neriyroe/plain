...
namespace plain {
    ...
    void Value::copy_from(const PLAIN_VALUE* source) {
        if(source == nullptr) { value = {}; return; }
        PLAIN_VALUE_COPY(&value, source);
    }

    Value::Value()                            { value = {}; }
    Value::Value(const Value& other)          { copy_from(&other.value); }
    Value::Value(Value&& other) noexcept      { value = other.value; other.value = {}; }
    Value::~Value()                           { clear(); }
    ...
    Value& Value::operator=(const Value& other) {
        if(this != &other) { clear(); copy_from(&other.value); }
        return *this;
    }

    Value& Value::operator=(Value&& other) noexcept {
        if(this != &other) { clear(); value = other.value; other.value = {}; }
        return *this;
    }
            ...
            case PLAIN_TYPE_INTEGER:
                if(value.data) {
                    snprintf(buffer, sizeof(buffer), "%d",
                             static_cast<int>(*(PLAIN_WORD_DOUBLE*)value.data));
                    return buffer;
                }
                return "0";
            case PLAIN_TYPE_REAL:
                if(value.data) {
                    snprintf(buffer, sizeof(buffer), "%g",
                             static_cast<double>(*(PLAIN_REAL*)value.data));
                    return buffer;
                }
                return "0";
