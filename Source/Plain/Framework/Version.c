...
MOCOSEL_WORD_DOUBLE MOCOSEL_VERSION(struct MOCOSEL_MANIFEST* manifest) {
    ...
    /* In case of unsupported platform or operating system, unexpected information may be stored. */
    if(manifest != NULL) {
    ...
    /* In case of unsupported platform or operating system, 0 shall be returned. */
    return 1U;
}
